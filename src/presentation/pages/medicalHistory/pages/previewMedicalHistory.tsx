...
export default function MedicalHistory() {
    ...
    // NUEVO: crear una nueva historia (navega al form)
    const handleCreate = () => {
        navigate("/medicalHistory/new")
    }
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between gap-2">
                        ...
                        <div className="flex gap-2">
                            {/* NUEVO botón crear */}
                            <Button className="bg-blue-100 text-black" onClick={handleCreate}>
                                Nueva historia
                            </Button>
                            <Button className="bg-green-100  text-black" variant="secondary" onClick={load} disabled={loading}>
                                {loading ? "Actualizando..." : "Actualizar"}
                            </Button>
                            <Button className="bg-red-100  text-black" variant="destructive" onClick={handleClearAll} disabled={!total}>
                                Limpiar todo
                            </Button>
                        </div>
                    </CardHeader>
                    <CardContent>
                        {loading ? (
                            ...
                            <div className="p-6 text-sm text-muted-foreground">
                                <div>No hay historias guardadas.</div>
                                {/* NUEVO: acción primaria en estado vacío */}
                                <div className="mt-3">
                                    <Button onClick={handleCreate}>Crear primera historia</Button>
                                </div>
                            </div>
                        ) : (
                            <Table>
                                ...
                                <TableBody>
                                    {ordered.map((r) => {
                                        ...
                                        // Tolerante a snake/camel por si cambian modelos
                                        const doc = patient?.document_number ?? patient?.documentNumber ?? ""
                                        const patientName =
                                            ...
                                            <TableRow key={r.id} className="hover:bg-muted/40">
                                                ...
                                                <TableCell className="text-right">
                                                    <div className="flex gap-2 justify-end">
                                                        ...
                                                        {/* <Button variant="outline" onClick={() => navigate(`/medical-history/${r.id}/edit`)}>Editar</Button> */}
